...
SekindoAdsManager.prototype.loadAdsProgram = function loadAdsProgram()
{
    ...
    function transferComplete(evt)
    {
        ...
        try
        {
            ...
            if(ref.adsProgram)
            {
                ...
                for(var i = 0; i < ref.newAdsProgram.length; i++)
                {
                    ref.newAdsProgram[i].impressionCount = 0;
                }
            }
            else
            {
                ref.adsProgram = json.ads;
                for(var i = 0; i < ref.adsProgram.length; i++)
                {
                    ref.adsProgram[i].impressionCount = 0;
                }
            }
    ...
    this.adsXmlhttp.withCredentials = true;
...
SekindoAdsManager.prototype.queryVasts = function queryVasts()
{
    ...
    var checkCounter = 0;
    function impressionCountCheck()
    {
        if(ref.adsProgramIndex < ref.adsProgram.length && ref.adsProgram[ref.adsProgramIndex].maxImpressions != -1 && ref.adsProgram[ref.adsProgramIndex].impressionCount >= ref.adsProgram[ref.adsProgramIndex].maxImpressions)
        {
            ref.adsProgramIndex++;
            checkCounter ++;
            impressionCountCheck();
        }
    }
    impressionCountCheck();
    if(checkCounter >= this.adsProgram.length)return;//We have no more ads to display
...
SekindoAdsManager.prototype.onVideoEvent = function onVideoEvent(val)
{
    ...
    switch(val.type)
    {
        case 'play':
            ...
            this.adsProgram[this.adsProgramIndex].impressionCount += 1;
...
SekindoUtils.isWindowInFocus = function(_window, callbackFunc)
{
    //Set the name of the hidden property and the change event for visibility
    //Note that Crome on ios does not support any event that can tell if the browser is minimized - for now it is not handled here.
    ...
    //Some browsers use visibilityChange only for tab shifting the following function coveres browser minimize in those cases.
...
function SekindoSPlayer(config, uniqueID)
{
    ...
    this.config.isPlaying = (this.config.isAutoPlay == true || this.config.isAutoPlay == 'true');
...
SekindoSPlayer.prototype.constructContainer = function constructContainer(iframe)
{
    ...
    this.config.mainDiv.style.marginLeft = 'auto';
    this.config.mainDiv.style.marginRight = 'auto';
    this.config.mainDiv.style.width = this.config.width + 'px';
...
SekindoSPlayer.prototype.constructPlayer = function constructPlayer()
{
    ...
    window['sekindoBus'+this.uniqueID].addCallBack('onPlay', function(val){ref.onUserEvent('onPlay', val);});
    window['sekindoBus'+this.uniqueID].addCallBack('onPause', function(val){ref.onUserEvent('onPause', val);});
    window['sekindoBus'+this.uniqueID].addCallBack('onMute', function(val){ref.onUserEvent('onMute', val);});
    ...
    SekindoUtils.isWindowInFocus(window, function(val){ref.isWindowInFocus(val);});
...
SekindoSPlayer.prototype.isWindowInFocus = function isWindowInFocus(val)
{
    var ref = this;
    if(!this.config.isPlaying)return;
    if(!val)
...
// <script type="text/javascript">
	...
	configPlayer.waterFall = 'http://live.sekindo.com/live/liveView.php?s=58057&vid_debugInfo=5728295_www.joyfulhealthyeats.com&vid_vastTimeout=-1&vid_content_url=https%3A%2F%2Fvideo.sekindo.com%2Fuploads%2Fvideo%2Ffood_myths_eating_late_mini.mp4&vid_content_id=food_myths_eating_late&vid_content_desc=Boosting+Your+Nutrition+IQ+-+Food+Myths%3A+Eating+Late&vid_content_title=Health+-+Food+Myths%3A+Eating+Late&vid_content_duration=84&vid_vastType=3&vid_waterfall=auto&vid_playerVer=&x=300&y=250&ri=&pubUrl=http%3A%2F%2Fwww.joyfulhealthyeats.com%2Fhealthy-meal-plan-week-27%2F&cbuster=${CBUSTER}' + '&vid_vpaidFlash=' + (supportFlashPlayerVersion ? '1' : '0');
	configPlayer.playlistMultiplier = 25; // (0 ...Infinity) - int
	configPlayer.contentClickUrl = '';
	configPlayer.contentPlayList = [{
				"url": "https://video.sekindo.com/uploads/video/food_myths_eating_late_mini.mp4",
				"title": "Health - Food Myths: Eating Late"
			}];
